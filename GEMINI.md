...
The core of the project is an **ensemble of RL agents**. Instead of relying on a single model, the system combines the signals from many different specialist agents to make more robust trading decisions.
...
First, install the required Python packages from the virtual environment. This includes libraries like PyTorch, Pandas, Stable Baselines3, and Matplotlib.
...
You can evaluate the performance of any combination of trained alphas using the `backtester.py` script.

```bash
# Backtest a single alpha
python backtester.py --alphas my_first_alpha
...
1.  Evaluate the performance of the current ensemble.
2.  Ask the LLM to invent a new, diverse alpha strategy.
3.  Train the new candidate alpha.
4.  Backtest the ensemble with the new candidate included.
5.  If performance improves, the candidate is permanently added to the portfolio. Otherwise, it is discarded and its details are added to a history file to prevent repetition.
...
*   **Modularity:** The code is organized into several modules, each with a specific responsibility:
    ...
    *   `train_alpha.py`: A script to train a single alpha strategy.
    *   `backtester.py`: A script to evaluate the performance of an ensemble of alphas.
    *   `orchestrator.py`: The main orchestration logic that evolves the ensemble.
...
*   **Alphas:** All trained and successful alphas are stored as subdirectories in the `alphas/` directory.
